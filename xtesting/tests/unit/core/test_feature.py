...
class FeatureTestingBase(unittest.TestCase):
    ...
    def _test_run(self, status, mock_mkdir, mock_method=None):
        ...
        mock_mkdir.assert_called_once_with(self.feature.res_dir, exist_ok=True)
    ...
    @mock.patch('time.time', side_effect=[1, 2])
    @mock.patch('os.makedirs')
    def _test_run_console(self, console, status, mock_mkdir, mock_method=None):
        self.assertEqual(
