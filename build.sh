...
for arch in ${arch}; do
    if [[ ${arch} == arm64 ]]; then
        ...
        find . -name Dockerfile -exec sed -i \
            -e "s|opnfv/xtesting:2026.1|${repo}/xtesting:arm64-${tag}|g" {} +
    elif [[ ${arch} == arm ]]; then
        find . -name Dockerfile -exec sed -i \
            -e "s|alpine:3.21|arm32v6/alpine:3.21|g" {} +
        find . -name Dockerfile -exec sed -i \
            -e "s|opnfv/xtesting:2026.1|${repo}/xtesting:arm-${tag}|g" {} +
    else
        find . -name Dockerfile -exec sed -i \
            -e "s|opnfv/xtesting:2026.1|${repo}/xtesting:amd64-${tag}|g" {} +
    fi
    dirs=${arch}_dirs
    for dir in ${!dirs}; do
